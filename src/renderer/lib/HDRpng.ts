...
function floatToRgb9_e5(buffer, res) {
	var r,
		...
		denom,
		s,
		l = (buffer.byteLength / 16) | 0,
		res = res || new Uint32Array(l);
	for (var i = 0; i < l; i++) {
		r = Math.min(32768.0, buffer[i * 4]);
		g = Math.min(32768.0, buffer[i * 4 + 1]);
		b = Math.min(32768.0, buffer[i * 4 + 2]);
...
function rgbeToFloat(buffer, res) {
	var s,
		l = buffer.byteLength >> 2,
		res = res || new Float32Array(l * 4);

	for (var i = 0; i < l; i++) {
		s = Math.pow(2, buffer[i * 3 + 3] - (128 + 8));
		res[i * 4 + 0] = buffer[i * 3] * s;
		res[i * 4 + 1] = buffer[i * 3 + 1] * s;
		res[i * 4 + 2] = buffer[i * 3 + 2] * s;
		res[i * 4 + 3] = 0.2;
	}
	return res;
}
